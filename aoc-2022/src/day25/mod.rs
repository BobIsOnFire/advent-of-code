...
fn convert_into_decimal(string: String) -> i64 {
    string.bytes().map(weird_digit_into_decimal).fold(0, |acc, num| acc * 5 + num as i64)
}
...
pub fn translate_numbers(lines: impl Iterator<Item = String>) -> util::GenericResult<(String, usize)> {
    let sum = lines.map(convert_into_decimal).sum();
